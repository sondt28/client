...
@Composable
fun GetAllScreen(modifier: Modifier = Modifier, dbService: IStudentAPI,  onBackPressed: () -> Unit) {
            ...
            items(students) { student ->
                Column(
                    ...
                    Row(
                        ...
                        Text(text = "${student.studentID}. ${student.firstName} ${student.lastName}", fontSize = 16.sp)
                        if (student.subjects.isEmpty()) {
                            Button(
                                onClick = {
                                    loadingStates[student.studentID] = true
                                    CoroutineScope(Dispatchers.IO).launch {
                                        val fullStudent = dbService.getSubjectByStudentId(student.studentID)
                                        withContext(Dispatchers.Main) {
